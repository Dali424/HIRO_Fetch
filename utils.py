...
class ReplayBuffer(object):
	...
	def episode_add(self):
		if len(self.episode_storage) == int(self.max_size / 10):
			self.episode_storage[int(self.episode_ptr)] = np.array(self.tmp_storage, dtype=object)
			self.episode_ptr = (self.episode_ptr + 1) % int(self.max_size / 10)
		else:
			self.episode_storage.append(np.array(self.tmp_storage, dtype=object))
		self.tmp_storage = []
