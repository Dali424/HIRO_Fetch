...
def evaluate_policy(policy, policy_high, eval_episodes=1):
    ...
    for _ in range(eval_episodes):
        ...
        high_input = np.concatenate((obs[0]['observation'], obs[0]['desired_goal']), axis=0)
        done = False
        ...
        while not done:
            ...
            while not done:
                ...
                low_input = np.concatenate((obs[0]['observation'], goal), axis=0)
                action = policy.select_action(low_input)
                new_obs, reward, turminated, truncated, done = env.step(action)

                avg_reward += reward
                low_reward += low_reward_cal(obs[0]['observation'], new_obs['observation'], goal)

                if -np.sum(abs(obs[0]['observation'] + goal - new_obs['observation'])) > args.reward_threshold:
                    print('goal reached')
                    obs = new_obs
                    break
...
def low_train(low_policy, replay_buffer, episode_timesteps, batch_size, discount, tau,
    ...
    is_success_values = []
    for item in d:
        is_success_values.append([item[0].item()['is_success']])
    # 추출된 값들이 리스트에 담겨 있습니다.
    done = torch.FloatTensor(is_success_values)  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_name", default="FetchReach-v2")  # OpenAI gym environment name
